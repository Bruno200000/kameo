...
const upsertPlatformSetting = async (key, value) => {
  const safeKey = encodeURIComponent(key);
  const existing = await supabaseFetch(`platform_settings?key=eq.${safeKey}&select=key&limit=1`);
...
app.patch('/api/admin/config', async (req, res) => {
  try {
    ...
    for (const key in updates) {
      await upsertPlatformSetting(key, updates[key]);
    }
    res.json({ success: true });
  } catch (err) {
...
app.use(async (req, res, next) => {
  try {
    if (req.path.startsWith('/api/auth') || req.path.startsWith('/api/admin') || req.path === '/api/settings') return next();
